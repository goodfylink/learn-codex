...
const SUB_AGENT_SYSTEM_PROMPT: &str = "You are a sub-agent assisting a primary agent. You have access to various tools. Perform the task assigned to you efficiently.";
...
impl ToolHandler for SpawnAgentHandler {
    ...
    async fn handle(&self, registry: Arc<ToolRegistry>, arguments: &str) -> String {
        ...
        let role = args.role.as_deref().unwrap_or("default");
        let agent = registry.spawn_agent_thread(role, SUB_AGENT_SYSTEM_PROMPT, &args.instruction);
        println!("[agent-team] spawned agent: {} [{}]", agent.id(), role);
...
impl ToolHandler for ListAgentsHandler {
    ...
    async fn handle(&self, registry: Arc<ToolRegistry>, _arguments: &str) -> String {
        tool_success(
            self.name(),
...
impl ToolHandler for SubAgentHandler {
    ...
    async fn handle(&self, registry: Arc<ToolRegistry>, arguments: &str) -> String {
        ...
        let agent =
            registry.spawn_agent_thread("default", SUB_AGENT_SYSTEM_PROMPT, &args.instruction);
